...
def test_create_runs(client, app, db_instance, celery_worker):
    """
    ...
    with mock.patch('monolith.views.auth.Client') as mocked:
        ...
        assert b'Hi email!' in rv.data
        assert b'Authorize Strava Access' in rv.data
        client.get('/strava_auth')  # calling strava_auth with the mocked instance of stravalib.client
        ...
        with mock.patch('monolith.background.c.ApiV3', side_effect=fun):
            rv = client.get('/fetch')
            body = json.loads(str(rv.data, 'utf8'))
            assert body['1'] == 2
            # nice so we created an user given him a fake token given him 2 fake runs fetched 'directly' from strava
