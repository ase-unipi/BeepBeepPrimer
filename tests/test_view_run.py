...
def test_non_existing_run(client):
    rv = client.get('/runs/1000')
...
def test_check_existing_run(client, app, db_instance, celery_worker):
    """
    ...
    with mock.patch('monolith.views.auth.Client') as mocked:
        ...
        with mock.patch('monolith.background.c.ApiV3', side_effect=fun):
            rv = client.get('/runs/154504250376823')
            body = json.loads(rv.data, 'utf8')
            assert body['1'] == 2
