...
def test_create_training_with_non_authenticated_user(client, celery_session_worker, db_instance):
    ...
    rv = client.post('/training_objectives', data=dict(submit='Publish', start_date=start_end_date, end_date=start_end_date, kilometers_to_run='1'), follow_redirects=True)
...
def test_create_training_starts_yesterday(client, background_app, db_instance):
    ...
    rv = client.post('/training_objectives', data=dict(submit='Publish', start_date=yesterday, end_date=today, kilometers_to_run='1'),follow_redirects=True)
...
def test_create_training_opposite_date(client, background_app, db_instance):
    ...
    rv = client.post('/training_objectives', data=dict(submit='Publish', start_date=start_date, end_date=end_date, kilometers_to_run='1'),follow_redirects=True)
...
def test_create_training_ends_yesterday(client, background_app, db_instance):
    ...
    rv = client.post('/training_objectives', data=dict(submit='Publish', start_date=start_date, end_date=yesterday, kilometers_to_run='1'),follow_redirects=True)
    assert rv.data.decode('ascii').count(str(start_date)) == 1
    assert rv.data.decode('ascii').count(str(yesterday)) == 1
    r = db_instance.session.query(Training_Objective).filter(Training_Objective.id == 1)
    assert r.count() == 0
...
def test_correct_assigning_trainig_runner(client, background_app, db_instance):
    ...
    client.post('/training_objectives', data=dict(submit='Publish', start_date=start_end_date, end_date=start_end_date, kilometers_to_run='1'),follow_redirects=True)
